...
def Upload():
    if request.method == 'POST':
        ...
        def download_all_objects_in_folder(xyz):
            ...
            objects = s3_resource.meta.client.list_objects_v2(Bucket=_BUCKET_NAME, Prefix=_PREFIX)




            #objects = list(my_bucket.objects.filter(Prefix=_PREFIX))
            ...
            for obj in objects:
                ...
                if not os.path.exists(newpath):
                    os.makedirs(newpath)
                my_bucket.download_file(obj.key, "InputFiles/" + filename)
            ...
            # resource= session.resource('s3').Bucket(_BUCKET_NAME).download_file(request.json['file'] + '/output/' + request.json['post_id'] +'finished.mp4',  'video_name/video.mp4')
